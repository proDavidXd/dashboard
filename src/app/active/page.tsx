...
import chartData from "@/utils/chart/active.json";
import LineChart from "@/components/Charts/Line";

const active = maindata[3];
  ...
  return (
    <div className="app">
      ...
      <div className="type flex gap-3 items-center m-5 justify-center">
        <span className="text-4xl">Active users</span>
        <Image src={active.icon} alt="type" width={32} height={32}></Image>
      </div>
      <main className="main flex flex-col items-center  p-10 ">
        <div className="now flex items-center gap-2">
          ...
          <span className="text-3xl font-semibold">{active.value}</span>
          <span className="text-xl">{active.unit}</span>
        </div>{" "}
        <br />
        <LineChart data={chartData} />
        <span className="text-xl">
          {active.growType}
          {active.grow}% from last month
        </span>
      </main>
    </div>
  );

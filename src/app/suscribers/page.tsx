...
import chartData from "@/utils/chart/suscribers.json";
import LineChart from "@/components/Charts/Line";

const suscribers = maindata[1];
  ...
  return (
    <div className="app">
      ...
      <div className="type flex gap-3 items-center m-5 justify-center">
        <span className="text-4xl">Suscribers</span>
        <Image src={suscribers.icon} alt="type" width={32} height={32}></Image>
      </div>
      <main className="main flex flex-col items-center  p-10 ">
        <div className="now flex items-center gap-2">
          ...
          <span className="text-3xl font-semibold">{suscribers.value}</span>
          <span className="text-xl">{suscribers.unit}</span>
        </div>{" "}
        <br />
        <LineChart data={chartData} className="chart md:w-1/2" />
        <span className="text-xl">
          {suscribers.growType}
          {suscribers.grow}% from last month
        </span>
      </main>
    </div>
  );

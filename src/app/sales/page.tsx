...
import chartData from "@/utils/chart/sales.json";
import LineChart from "@/components/Charts/Line";

const sales = maindata[2];
  ...
  return (
    <div className="app">
      ...
      <div className="type flex gap-3 items-center m-5 justify-center">
        <span className="text-4xl">Sales</span>
        <Image src={sales.icon} alt="type" width={32} height={32}></Image>
      </div>
      <main className="main flex flex-col items-center  p-10 ">
        <div className="now flex items-center gap-2">
          ...
          <span className="text-3xl font-semibold">{sales.value}</span>
          <span className="text-xl">{sales.unit}</span>
        </div>{" "}
        <br />
        <LineChart data={chartData} className="chart md:w-1/2" />
        <span className="text-xl">
          {sales.growType}
          {sales.grow}% from last month
        </span>
      </main>
    </div>
  );
